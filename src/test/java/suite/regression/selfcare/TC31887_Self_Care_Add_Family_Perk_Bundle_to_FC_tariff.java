...
public class TC31887_Self_Care_Add_Family_Perk_Bundle_to_FC_tariff extends BaseTest {
    ...
    public void TC31887_Self_Care_Add_Family_Perk_Bundle_to_FC_tariff() {
        ...
        owsActions.createOrderOnlineCCCustomerWith2FC1BundleAndNK2720(TC31887_CREATE_ORDER);

        test.get().info("Step 2 : Create the new billing group");
        BaseTest.createNewBillingGroupMinus15Days();
        ...
        List<HashMap<String, String>> temp = ServiceOrderEntity.dataServiceOrder(serviceRefOf1stSubscription, "Change Bundle", "Completed Task");
        int size = ServiceOrdersContentPage.getInstance().findARowInTable(temp);
        ...
        Assert.assertEquals(3, TasksContentPage.TaskPage.EventsGridSectionPage.getInstance().countNumberOfEvents(EventEntity.dataForEventChangeBundle("PPB: AddSubscription: Request completed", "Completed Task")));
        Assert.assertEquals(1, TasksContentPage.TaskPage.EventsGridSectionPage.getInstance().countNumberOfEvents(EventEntity.dataForEventChangeBundle("Service Order Completed", "Completed Task")));
        ...
        List<HashMap<String, String>> otherProducts = OtherProductEntiy.dataForAOtherBundleProduct("BUNDLER - [500-FONMIN-0-FC]", "Bundle", "Discount Bundle Recurring - [Family perk - 500 Tesco Mobile only minutes per month]", "£0.00", newStartDate);
        ...
        Assert.assertEquals(1, otherProductsGridSectionPage.countNumberOfOtherProducts(otherProducts));
